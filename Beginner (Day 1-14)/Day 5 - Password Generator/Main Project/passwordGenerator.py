...
password = "" 
for index in range(0,len(passList)):
    keyword = random.choice(passList) #get random char from the list
    passList.remove(keyword) #remove char so it doesn't repeat
    password += keyword #append each char to the string variable
